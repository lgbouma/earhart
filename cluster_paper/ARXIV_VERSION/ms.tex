...
close enough ($d\approx400$\,pc) to facilitate measurements of stellar rotation periods using TESS.  \citet{healy_stellar_2020}
in fact already used TESS rotation periods to study the stellar inclination
distribution of stars in the core of NGC\,2516.
The cluster's halo however was reported by
\citet{kounkel_untangling_2019} to span
$\approx20\times10\times350$\,pc, which is a larger volume than
...
To assess whether the stars in the cluster's halo are the same
...
the Lomb-Scargle periodogram implementation in
\texttt{astrobase} \citep{lomb_1976,scargle_studies_1982,bhatti_astrobase_2018}.
...
$\mathcal{A}$ and $\mathcal{B}$.  For Set
...
stars, out of \nautorotdenominator\ stars that met the initial crowding requirements.  For Set $\mathcal{B}$, we
additionally required that the Lomb-Scargle period
...
The rationale behind the additional cut in Set $\mathcal{B}$ is
...
($0.5<\bpmrpo<1.3$), there are 523 stars, of which
174 (33\%) show signs of binarity.  Dividing these
523 stars into ``fast'' and ``slow'' sequences by eye,
the fraction of stars showing signs of binarity in the slow and fast
subsamples are 27\% (106/289) and 51\% (68/134), respectively.  We speculate on possible
...
\begin{figure*}[tp]
  ...
  rotation periods in Set~$\mathcal{B}$ are shown in blue;  halo
  ...
  ratio of detected to expected stars with rotation periods in Set $\mathcal{B}$ is shown
  ...
  velocity space (from Set~$\mathcal{A})$.
...
Figure~\ref{fig:physical_x_rotn}.  Alternative visualizations
...
all these figures, to ensure a fair comparison, we only show
...
of stars were in each bin.  Rotation periods consistent with a
gyrochronological age of 150\,Myr are detected for
68\% (471/692) of stars reported to be in the cluster.  The average
detection rate inside of 25\,pc, 72\% (290/404), is
slightly higher than outside of 25\,pc, where the detection
rate is 63\% (181/288). Within $\approx$3.5\,pc, the period detection
rate of 59\% (26/44) is
anomalously low, due to crowding and saturation
in the TESS images.
...
rotation periods in Set $\mathcal{A}$, $\approx$8 appear as
...
\begin{figure*}[t]
	...
	\caption{ {\bf Lithium and rotation in NGC\,2516.} Points show all
		candidate \cn\ members with TESS rotation periods (Set $\mathcal{A}$) and Gaia-ESO or
...
\begin{figure*}[t]
  ...
  results of \citet{Irwin_NGC2516_2007}
  (I07), \citet{fritzewski_rotation_2020} (F20), and
  \citet{healy_stellar_2020} (HM20).
  I07, F20, and HM20 analyzed stars in the
  core of \cn.
  \label{fig:litcomp}
	}
\end{figure*}
...
slow and fast rotation subsamples are 26\% (106/289) and 51\% (68/134); the fast rotators show a
preference for binarity.  This correlation is in line with the earlier
findings noted above.  While many rapid rotators in \cn\
(66\%) have no evidence for being binaries, the cuts we
...
\begin{itemize}
  ...
  \item {\it Bonafide \cn\ members exist out to $\approx$250\,pc in
    ...
    assessment.
   	The majority of the halo members do however exist within
  ...
  \item {\it The field star contamination rate increases at larger
    ...
    $\approx$50\%.  While the latter contamination fraction may seem high, a comparison sample of field stars (Appendix~\ref{app:compstar}) has a detection rate three times lower, and a  distribution in the period-color plane inconsistent with that of a population with a single age.
   %
...
\begin{figure}[t]
  ...
  The dashed line is as in Figure~\ref{fig:rot}.
  \label{fig:compstar}
	}
\end{figure}
...
Within $0.4<\bpmrpo<0.8$, the same requirements
for crowding resulted in \ncompstardenominator\ field stars
for which rotation periods could have been detected.  Imposing the
same Lomb-Scargle power cutoff (${\rm LSP}>0.08$) and color-period cutoff as that used to
define Set $\mathcal{B}$ yielded \ncompstarnumerator\ period
detections (\ncompfrac).  Within the same color range, \nautovscompstarnumerator\ of
\nautovscompstardenominator\ kinematically
identified candidate cluster members yielded period detections
(\nautofrac).  The period--color distributions
are also quite different: the candidate
...
Figure~\ref{fig:gaia6d_x_rotn} is an alternative visualization
