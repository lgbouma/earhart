...
\begin{figure*}[t]
  ...
  cluster orbit.
  In different projections, the gray line spans different
...
being coeval is to analyze the Gaia Hertzsprung-Russell (HR) diagrams.
Comparable analyses have already been performed by
\citetalias{cantatgaudin_gaia_2018},
\citetalias{kounkel_untangling_2019}, and \citetalias{meingast_2021}.
...
show a consistent sequence, though with greater scatter.  One possible
explanation for the additional scatter could be that the halo is more
contaminated by field stars.  For instance, $\approx5$ halo
stars are present in portions of the red giant branch
(RGB) that imply stellar ages inconsistent with that of the cluster.
These stars must be field interlopers.
...
0.45\,$M_\odot$.  Below this mass, the data and models diverge at
...
are consistent.  The left panel of Figure~\ref{fig:hr} shows that
...
models, but are bluer than the 100 Myr model.  The RGB stars are
...
adopt an PARSEC-based isochrone age for \cn\ of $165\pm35$\,Myr, where
our quoted uncertainty is a quadrature sum of their quoted statistical
and systematic components.  The relative age of ``slightly older than
...
\begin{figure*}[t]
  ...
  \caption{ {\bf TESS rotations periods and dereddened Gaia colors for \cn.}
    {\it Top row}: \cn\ split into its core and halo.  Subset
    $\mathcal{A}$ ({\it left columns}) and $\mathcal{B}$ ({\it right
    columns}) undergo successive degrees of automated cleaning,
    described in Section~\ref{subsubsec:cluster}.  The Pleiades
    ...
    There is a narrow range of color, from $1.2<(Bp-Rp)_0<1.7$, in
    which some NGC\,2516 members appear slightly elevated with respect
    to the Pleiades, suggesting an older age.
    {\it Bottom row}: \cn\ colored by binarity indicators.  Sources
    with $\mathrm{RUWE}>1.2$ appear in red (astrometric binaries; 11\%
    of stars).
    Overplotted in orange are sources at least 0.3\,mag above an
    empirically fit cluster isochrone (photometric binaries; 20\% of stars).
		\label{fig:rot}
	}
\end{figure*}
...
periods, following three different sets of cleaning criteria.  The
first and second sets of criteria were entirely automated, and yielded
subsets we denote $\mathcal{A}$ and $\mathcal{B}$.  For both subsets,
we considered light curves for which the peak Lomb-Scargle periodogram
period was below 15 days, the normalized Lomb-Scargle power exceeded
0.08, and for which no equal-brightness or greater companions were
within the aperture.  Beyond requiring that the target star be the
brightest star in the aperture, we also required that at most one
companion with brightness exceeding one tenth of the target star's
brightness be present in the aperture according to the Gaia DR2 source
catalog.  For set $\mathcal{A}$, this yielded \nautorotnumerator light
curves, out of \nautorotdenominator stars for which rotation periods
might plausibly have been detected.  For set $\mathcal{B}$, we 
...
While subset $\mathcal{A}$ is more complete than subset $\mathcal{B}$,
this completeness comes at the expense of greater contamination.
% For instance, stars above the slow sequence (possible field
% contaminants) for the most part disappear in subset $\mathcal{B}$

The comparison to the Pleiades and Praesepe is made using the rotation
periods and dereddened colors from Table~5 of
\citet{curtis_rup147_2020}, which used data from
...
% the slow sequence, which could be field contaminants, for the
...
%FIXME this belongs in the DISCUSSION!!
%FIXME this belongs in the DISCUSSION!!
%FIXME this belongs in the DISCUSSION!!
%FIXME this belongs in the DISCUSSION!!
Comparing the slow sequence of the Pleiades and \cn\ 
more closely, the top row of Figure~\ref{fig:rot} shows that the two
sequences
overlap from roughly $0.5<(Bp-Rp)_0<1.2$.  At redder colors
($1.2<(Bp-Rp)_0<1.7$) the dispersion in rotation periods increases.
The maximum rotation periods seen in \cn, in both subsets
$\mathcal{A}$ and $\mathcal{B}$ extend up to $\approx$11 days, rather
than the $\approx$8.5day limit seen in the Pleiades.  This is
consistent with \cn\ being slightly older than the Pleiades.  Fitting
a model to substantiate this statement \citep[{\it e.g.}, any
of][]{mamajek_improved_2008,angus_toward_2019,spada_competing_2020} is
an exercise in tautology, since the aforementioned models are empirically fit using
the Pleiades and Praesepe.  In other words, at $(Bp-Rp)_0=1.35$, we
see rotation periods in \cn\ ranging from 7 to 10 days.  The 7 day
periods overlap with the Pleiades; the 10 day periods do not.  Fitting
formulae from the previously cited studies give ages for
a star with $(Bp-Rp)_0=1.35, (B-V)_0=1.10, P=7\,{\rm day}$ of 204 Myr,
316 Myr, and 107 Myr respectively.  Therefore, only the
\citet{spada_competing_2020} model is well-calibrated at these ages
and colors.  For an 8 day rotation period of a star with the same
color, this model quotes a 193 Myr age; at 9 days, 388 Myr.
Given the model uncertainty, we prefer the relative statements that
\cn\ appears slightly gyrochronologically older than the Pleaides, and
much younger than Praesepe.
...
1.2 as an astrometric binary (11\% of the overall cluster sample).  A
cautionary point is that poor astrometric fits can also be caused by
photometric variability.  To assess photometric binarity, we fitted a
spline to Figure~\ref{fig:hr} by hand, and labelled any points
brighter than 0.3\,mag above the cluster sequence as photometric
binaries.  This included 20\% of the overall cluster sample.  Many of
the resulting candidate binaries, though not all of them, appear
underneath the ``slow sequence''.  We speculate on possible
...
% some wide separation pairs and dist_pc:
% 5263077805266480000  5506439929389838592  421.083675
% 5506623616550497792  5273740456715689088  427.053947
% 5506439929389838592  5213536216500903680  427.557268
% 5273740456715689088  5488984598182200704  428.952934
% 5506439929389838592  5273740456715689088  438.906581
...
NGC2516, PscEri concatenation?)
  ...
  %Why is there a slow and fast sequence?
