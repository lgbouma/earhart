...
% 	\end{center}
% 	\vspace{-0.7cm}
%   \caption{ {\bf TESS rotations periods versus color for \cn.} {\it
%   Top row}: The core and halo of \cn.  Sets $\mathcal{A}$ ({\it left})
%   and $\mathcal{B}$ ({\it right}) undergo successive stages of
%   cleaning (see Section~\ref{subsubsec:cluster}).  The Pleiades
...
%   members have longer rotation periods than Pleiades members.  {\it
%   Bottom row}: Binarity indicators in \cn.  Astrometric binaries
%   ($\mathrm{RUWE}>1.2$; 11\% of stars) appear in red; photometric
%   binaries ($>$0.3\,mag from an empirical isochrone; 20\% of stars)
%   are overplotted in orange.  Rotation periods for comparison field
%   stars are shown in Appendix~\ref{app:compstar}.
%   \label{fig:rot}
% 	}
% \end{figure*}
...
\begin{figure*}[t]
	\begin{center}
		\leavevmode
		\subfloat{
			\includegraphics[width=0.72\textwidth]{f3a.pdf}
		}

    \vspace{-0.7cm}
		\subfloat{
			\includegraphics[width=0.72\textwidth]{f3b.pdf}
		}
	\end{center}
	\vspace{-0.7cm}
...
\begin{figure*}[t]
	\begin{center}
		\leavevmode
		\subfloat{
			\includegraphics[width=0.72\textwidth]{f3c.pdf}
		}

    \vspace{-0.5cm}
		\subfloat{
			\includegraphics[width=0.72\textwidth]{f3d.pdf}
		}
	\end{center}
	\vspace{-0.7cm}
...
4000\,{\rm K}$ tend to have non-zero equivalent widths, despite the
...
ten degrees from the galactic plane.  Special targeting for stars in
the TESS southern continuous viewing zone, and for known open cluster
members was also performed.  We identified the candidate \cn\ members
...
% Li\,6708\,\AA\ doublet, these were the most relevant bit flags.
...
spectra to the stellar rest frames, and then focused in on a 15\AA\
window centered on the 6707.835\AA\ lithium doublet.  We then
...
distribution, and are included in Table~\ref{tab:maintable}.  We
verified the overall scale of our results by comparing our
measurements with those of \citet{randich_gaiaeso_2018}.  Most stars
with $T_{\rm eff} > 4500\,{\rm K}$ followed a 1-1 relation.  For
cooler stars, our EW measurements are systematically lower, as
...
\citet{pecaut_mamajek_2013} table).  For clarity, we have omitted the
...
alter the result though, since the innermost and outermost are likely
...
\begin{figure*}[t]
  ...
  1,099 stars, are shown in 1\,mag bins.  The mean star in the
...
\begin{itemize}
  ...
  \item {\it The field star contamination rate increases at larger
    ...
    quantify the statement: $\approx$70\% of the inner-most
    members have detected rotation periods consistent with a
   ...
   Section~\ref{subsec:origin}.
   While the mass function is consistent with expectations for a
   ...
   \item {\it The rapid rotators show elevated lithium abundances, and
     ...
     would be expected to produce less efficient internal mixing and a
...
open cluster dispersal.
Combined with wide-field photometric surveys, the stars themselves
will enable advances in understanding stellar
angular momentum evolution.
Combined with wide-field spectroscopic
surveys \citep[{\it e.g.},][]{kollmeier_2017}, the halos could also lead to
advances in understanding the chemical evolution of low-mass stars.
Finally, the halo stars also provide an expanded sample of
age-dated stars to search for planets \citep[{\it
e.g.},][]{newton_2021}, which could help expand our understanding of
planetary evolution.
