...
% ... which came from ~/Dropbox/proj/cdips/data/cluster_data/cdips_catalog_split/OC_MG_FINAL_v0.4_publishable_CUT_NGC_2516.csv
...
%\newcommand{\nautorot}{1{,}144\ } % % For nequal<=0, nclose<=100, LSP>0.08. See /doc/20210205_nequal_and_Prot_vs_color_cleanup.txt

\newcommand{\nkinematic}{3{,}018\ } % "fullfaint" kinematic sample, core+halo. from any call of earhat.helpers._get_fullfaint_dataframes
\newcommand{\ncdips}{2,238\ } % e.g., from OC_MG_FINAL_v0.4_publishable_CUT_NGC_2516.csv. checked with plot_target_vs_actual_list_sanity_checks.py
...
\begin{abstract}
  ...
  around the $\approx$175\,Myr open cluster NGC\,2516.
  %TODO: says who?
...
\citep{krumholz_star_2019}.  Since only a small fraction of the cloud
mass is converted into stars ($\sim 1\%$), gas dispersal enables the
...
suffer an onslaught that almost always leads to dissolution.  Within
...
the galactic field is of interest for a few reasons.  One is to
understand the predominant conditions under which stars and star
clusters form \citep[{\it e.g.},][]{wright_kinematics_2018}.  Another
...
The converse holds too: one could use chemical tagging to test the
...
The resuls can be summarized as follows. On one end are coeval
populations with no discernable cores ({\it e.g.}, Psc Eri and $\mu$
Tau,
\citealt{meingast_psceri_2019,curtis_tess_2019,gagne_mutau_2020}).  On
the other are hierarchically structured populations with many over and
under-densities  \citep[{\it e.g.} the Sco-Cen and Vela OB2
associations][]{pecaut_star_2016,cantatgaudin_velaOB2_2019}.  Here, we
focus on the middle: low-density halos associated with a single
...
\citealt{kounkel_untangling_2019}, hereafter
\citetalias{kounkel_untangling_2019}], and \citealt{meingast_2021},
hereafter \citetalias{meingast_2021}).  Discrepancies between
different clustering methods leads to the question of how contaminated
the resulting samples are, and in some cases whether the reported
structures are truly coeval stellar populations.

We have recently been making TESS light curves of stars reported to be
members of such populations (CDIPS, \citealt{bouma_cdipsI_2019}).  Our
analysis of TESS Cycle 1 (Sectors 1-13) yielded light curves of
\numsouthernuniqlcs candidate cluster members in the Southern Ecliptic
hemisphere, available on
MAST\footnote{\url{https://archive.stsci.edu/hlsp/cdips}}.

As part of this broader project, we focus here on a modest question:
in just a single rich southern open cluster, is the cluster halo truly
coeval with the core?  We chose NGC\,2516 for this analysis since it
is young ($\approx$120\,Myr) and sufficiently nearby
($d\approx400$\,pc) to facilitate photometric rotation measurements
using TESS, and some degree of spectroscopic analysis.  Its halo is
decidedly non-spherical: \citetalias{kounkel_untangling_2019} reported
its dimensions as $\approx20\times10\times350$\,pc.  We want to know:
is the halo real? To what extent can we use Gaia alone to reliably
identify age-dated needles in the haystack of field stars?  What are
the implications, observationally and theoretically, if we can
identify the dispersed halos of open clusters?
...
being in ``halos'', ``strings'', ``coronae'', ``snakes'', ``the
...
association.  We opt to use ``halo'' because it is concise and model
agnostic.  It suffers however from the suggestion of spherical
symmetry.  The halo of NGC\,2516 is anything but.  
...
\begin{figure*}[t]
	...
	\end{center}
	\vspace{-0.7cm}
  \caption{ {\bf The dense and diffuse components of NGC\,2516 across position and
  velocity space.} The core (black) was identified by
  \citet{cantatgaudin_gaia_2018} using Gaia DR2, and is coincident with
  the visual overdensity of stars canonically accepted as ``the
  cluster''.  The halo (blue) was identified by
  \citet{kounkel_untangling_2019} and \citet{meingast_2021} using less restrictive membership
  assignment algorithms described in Appendix~\ref{app:clustering}.  The field
  sample is randomly drawn from a $(\alpha, \delta, \pi)$ cube
  centered on the cluster.  The low volume density of the
  halo stars makes it difficult to visually distinguish
  the field and the halo samples.
  \label{fig:gaia6d}
	}
\end{figure*}

We selected candidate \cn\ members based on those available in the
literature.  While a number of pre-Gaia catalogs were available, the
...
and \citet{meingast_2021} (\citetalias{meingast_2021}).
A useful visualization of these samples is available online\footnote{
  \url{https://homepage.univie.ac.at/stefan.meingast/coronae.html},
  made by \citet{meingast_2021}.  }.
While we could have performed our own clustering analysis based on the
Gaia data, such an effort would be only replicating the work of these
investigators.  We opt to instead use their studies as starting
points.

%FIXME FIXME FIXME FROM HERE
...
$\approx$1\,mag fainter.  
Finally, 1577 of the 1860 \citet{meingast_2021} sources (85\%)
were included in the \citet{kounkel_untangling_2019} sample.  
Each study applied unsupervised clustering techniques
to the second Gaia Data Release, as detailed in
Appendix~\ref{app:clustering}.


%FIXME 
\citetalias{cantatgaudin_gaia_2018} considered a cone of radius {\bf
...
\begin{figure*}[t]
  ...
  \caption{ {\bf HR diagrams of NGC\,2516.}
    {\it Left}: The photometric data are from Gaia EDR3; no reddening correction
    ...
    consistent with these stars having not yet reached the
    main-sequence.
    {\it Right}: Solar-metallicity PARSEC isochrones are overplotted
    at intervals of $\log_{10}(t/\mathrm{yr})=\{8, 8.25, 8.5\}$.
    The arrow represents the average reddening correction applied
    to the models.
    The main-sequence turn-off location and evolved stars suggests a
    cluster age slightly older than the Pleiades.  The models diverge
    from the data for $M_\star \lesssim 0.45M_\odot$ ($\approx$M2V;
    $\mathrm{Bp}-\mathrm{Rp}\approx2.2$).
    \label{fig:hr}
  }
\end{figure*}

The first check on the plausibility of the candidate cluster members
being coeval was already performed by
\citetalias{cantatgaudin_gaia_2018},
\citetalias{kounkel_untangling_2019}, and \citetalias{meingast_2021}.
That check is to see whether the HR diagrams of the cluster members
support the claim that they are coeval.
...
consistent main-sequence, perhaps with greater scatter.  One
possible explanation for the additional scatter could be that the halo
is more contaminated by field stars.  
For instance, $\approx5$ evolved halo stars are visible in the
field-star portion of the red giant branch (RGB).
Given the expected age of the system, these stars must be field
interlopers.

A separate possible explanation for scatter in the halo's HR diagram
could be differential reddening.  The halo is reported to span
20$^\circ$ on-sky, and varies in position from about $b=-12^\circ$ to
...
Some empirical evidence for differential reddening is discussed
further in Appendix~\ref{app:diffred}. Based on the available
evidence, we expect that both field star contamination and
differential reddening could play a role.
A third possibility that we have not explored is whether the binary
fraction could differ in spatially different regions of the cluster.

In the right panel of Figure~\ref{fig:hr}, we compare the observed
photometry with PARSEC isochrones
...
\texttt{2021-02-26}, \texttt{CMD3.4}} to interpolate these isochrones
at $\log_{10}(t/\mathrm{yr})=\{8, 8.25, 8.5\}$, and applied the
reddening correction described in Section~\ref{subsec:tess}. This
entailed by including using our corrected value of
$E(B-V)=0.102\pm0.019$, assuming $R_V=3.1$, and applying the
\citet{odonnell_1994} SED-dependent reddening law.
The data and models generally agree for masses above
approximately 0.45\,$M_\odot$.  Below this mass, at colors redder than
Bp-Rp$\approx2.2$, the data and models diverge.  The MIST isochrones show a
comparable disagreement \citep{choi_mesa_2016}.  Explanations invoking
both starspots and incomplete molecular line lists have been proposed
\citep[{\it
e.g.},][]{stauffer_why_2003,feiden_magnetic_2013,rajpurohit_effective_2013,mann_spectrothermometry_2013,choi_mesa_2016}.
We adopt the 
PARSEC isochrones since they diverge at slightly lower mass than the
MIST models, due to the temperature-opacity calibration
discussed by \citet{chen_improving_2014}.
For purposes of age-dating the cluster, we prefer to simply focus on
the main-sequence turn-off (MSTO), since it is where the models have
...
are consistent.  The left panel of Figure~\ref{fig:hr} shows
empirically that they seem to be: stars past the turnoff in both the
halo and core are on a consistent locus.  The right panel of
Figure~\ref{fig:hr} suggests the precision with which the claim can be
made.  The MSTO stars are consistent with the 178 and 316 Myr models,
but are bluer than the 100 Myr model.  The RGB stars are consistent
only with the 178 Myr ($\log_{10} t/{\rm yr} = 8.25$) model.  Based on
the assumption that the width of the MSTO can be attributed to binary
stars, we are most interested in the blue edge. This seems most
compatible with the 178 Myr model.
...
% yields $XXX\pm YYY$\,Myr.  And finally, applying the same procedure to
