...
%FIXME these change depending on your cutoff. ensure the final numbers agree!
...
\begin{abstract}
  ...
  %TODO: Says who? Says Piskunov+08
...
% NOTE: words in this paragraph need work
...
and under-densities  \citep[{\it e.g.} Sco-Cen and Vela
OB2][]{pecaut_star_2016,cantatgaudin_velaOB2_2019}.  Here, we focus on
...
Difference between different clustering methods raises the issue of
...
Our analysis of TESS Cycle 1 (Sectors 1-13) yielded light curves of
...
symmetry.  The halo of NGC\,2516 might be better-described as ``tidal
tails'', but the primary goal of this study however is
not to verify this claim.
...
of the art.  We therefore adopted what we viewed as the most
...
available online\footnote{
  ...
  made by \citet{meingast_2021}.}.  While we considering performing
...
The Gaia clustering studies each used slightly different selection
functions, with different results.
\citetalias{cantatgaudin_gaia_2018} considered stars brighter than
$G=18$\,mag within a few degrees of the cluster center, and reported
1106 candidate members of \cn.  \citetalias{kounkel_untangling_2019}
and \citetalias{meingast_2021} considered stars up to $\approx$1\,mag
fainter, and reported 3003 and 1860 members respectively.  The
unsupervised clustering techniques that each of these studies applied
to the second Gaia Data Release are discussed
Appendix~\ref{app:clustering}, as is the overlap between their
resulting membership samples.
...
\citetalias{kounkel_untangling_2019} and \citetalias{meingast_2021}
members span tens of degrees.  In physical units, the physical size of
the entire structure is $\approx20\times10\times350$\,pc.
As a side note, stars in the upper right quadrant of $\mu_{\alpha'}$
{\it vs.} $\mu_\delta$ were reported almost entirely by
\citet{meingast_2021}.
...
\citetalias{kounkel_untangling_2019} and \citetalias{meingast_2021}
members as the ``halo''.  These definitions imply that there are
\ncore core members, and \nhalo halo members.  This distinction is
...
cluster center.  Nonetheless, the membership catalog of
\citetalias{cantatgaudin_gaia_2018} echoes that of many previous
investigators (CITE, CITE, CITE), and is consistent with the general
{\it visual} impression that one has when looking at \cn\ in the night
sky: it appears to span $\approx$2$^\circ$, and not $\approx20^\circ$.
...
the \texttt{astroquery.gaia} module (CITE).  We allowed the number of
stars in the comparison sample to exceed that in the cluster sample by
a factor of $\approx$5, to ensure broad sampling of stellar masses and
evolutionary states.  We also required the comparison sample to not
overlap with the cluster sample, which led to the omission of 1.1\% of
the stars drawn from the volume noted above.
...
at colors redder than Bp-Rp$\approx2.2$.  The MIST isochrones showed a
...
\begin{figure*}[h]
  ...
  outcomes.} 3298 unique candidate cluster members found
