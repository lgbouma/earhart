...
literature.  While some pre-Gaia analyses were available, the purity
and accuracy of the Gaia-derived results are the current state of the
art \citep{jeffries_ngc2516_2001,Kharchenko_et_al_2013}.  We therefore
adopted what we viewed as the most interesting clustering samples to
compare: those of \citet{cantatgaudin_gaia_2018},
...
\begin{figure*}[t]
  ...
  \caption{ {\bf TESS rotations periods and dereddened Gaia colors for \cn.}
    ...
    Stars redder than $(Bp-Rp)_0\gtrsim 2.2$ (later than M2V) 
    are fainter than the magnitude limit of our survey.
    {\it Bottom row}: \cn\
...
source list had $Rp<16$.  The difference is due to 34 stars unique to
\citet{meingast_2021} which were not available at the time of the
CDIPS reductions, and a few stars falling on the TESS chip gaps.  The
...
Appendix~\ref{app:detrending}.  After applying an initial detrending
step aimed at removing systematic trends, we proceeding with a few
small cleaning steps to improve the purity of the rotation period
measurements: we masked 0.7 days at the beginning and end of each
...
using the periodogram implementations in \texttt{astrobase}, in
particular the Stellingwerf phase-dispersion minimization periodogram,
along with the more traditional Lomb-Scargle
...
To clean the resulting rotation period measurements, we devised a set
...
contamination.  For instance, stars above the slow sequence (possible
field contaminants) for the most part disappear in set $\mathcal{B}$.
...
of \cn\ are assessed in greater detail in Section~\ref{disc:absage}.
...
photometric binarity, we fitted a spline to Figure~\ref{fig:hr} by
hand, and labelled any points brighter than 0.3\,mag above the cluster
...
all of them, appear underneath the ``slow sequence''.  We speculate on
...
\citet{fritzewski_rotation_2020} have recently solidified our
understanding that at ages between 100 and 200\,Myr, young open
clusters ({Blanco\,1, the Pleiades, Psc\,Eri, M\,35, and M\,50) all
seem to show similar rotation period distributions.  The FGK stars
show a ``slow sequence'', a population of ``rapid rotators'', and
in some cases stars in the ``void'' between the two
populations ({\it e.g.,}, Psc\,Eri and \cn\ show such populations).
The late M dwarfs with detected rotation periods tend to rotate very
rapidly ($<3$\,days) --- these are unfortunately beyond the magnitude
limit of the present analysis.  In this section, we briefly discuss
these features, and focus on the main difference between \cn\
and the Pleiades: the longer maximum rotation periods seen in the
K-dwarfs.

\subsubsection{The slow and fast sequences}
...
\citet{skumanich_time_1972} has not worked in detail, the fact that it
is simple and can be understood to first order through magnetic
breaking \citep{weber_angular_1967} has led to more systematic efforts
at age-dating stars through their rotation periods.
...
answered.  Models that introduces variable core-envelope decoupling
...
random change of magnetic field topology that cause the dynamo to
...
A separate hypothesis that we can test from the existing data is the
idea that 
the rapid rotators are somehow linked to binarity.
Analyses by {\it e.g.}, \citet{gillen_ngts_2020} and
\citet{simonian_rapid_2019} have reported that
rapid rotators in young clusters and in the field are correlated
with binarity.
Related effects could include tidal synchronization for the shortest
period binaries,
or some form of disk truncation that when combined with magnetic locking
(CITE)
could produce the rotation period anomaly.
The lower panels of Figure~\ref{fig:rot} show our best attempt to
identify unresolved binaries in the \cn\ rotation sample.
Binaries resolved by Gaia were already excluded in our initial
definitions of the samples.
WHAT IS THE FRACTION OF FAST ROTATORS THAT ARE BINARY?
...
from roughly $0.5<(Bp-Rp)_0<1.2$.  At redder colors
