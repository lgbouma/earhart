...
\begin{figure*}[t]
  ...
  \caption{ {\bf TESS rotations periods and dereddened Gaia colors for \cn.}
    ...
    $\mathcal{A}$ ({\it left columns}) and $\mathcal{B}$ ({\it right columns}) undergo
    successive degrees of automated cleaning, described in
    ...
    in which some NGC\,2516 members appear slightly elevated with respect to
    the Pleiades, suggesting an older age.
    {\it Bottom row}: \cn\ colored by binarity indicators.
    Sources with $\mathrm{RUWE}>1.2$ appear in red (astrometric
    binaries).
    Overplotted in orange are sources at least 0.3\,mag above an
    empirically fit cluster isochrone (photometric binaries).
		\label{fig:rot}
	}
\end{figure*}
...
points colored by indicators of binarity.
To assess astrometric binarity, we used the renormalized unit weight
error (RUWE) from Gaia EDR3.
For plotting purposes, we labeled anything with RUWE exceeding 1.2 as
an astrometric binary (11\% of the overall cluster
sample).
A cautionary point is that poor astrometric fits can also be caused by
photometric variability.
To assess photometric binarity, we fitted a spline to
Figure~\ref{fig:hr} by hand, and labelled any points brighter than
0.3\,mag above the cluster sequence as photometric binaries.
This included 20\% of the overall cluster sample.
Many of the resulting candidate binaries, though not all of them,
appear underneath the ``slow sequence''.
We speculate on possible explanations in Section~\ref{disc:rotn}.
