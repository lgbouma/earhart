...
\newcommand{\nautorotnumeratormatching}{892} % plot_autorotation.py, Set A, with manual Prot-color cut

%\newcommand{\ncompstardenominator}{820} % plot_compstar_rotation.py
%\newcommand{\ncompstarnumerator}{312} % plot_compstar_rotation.py

\newcommand{\ncompstardenominator}{656} % plot_compstar_rotation.py BpmRp0 from 0.4 to 0.8
%\newcommand{\ncompstarnumerator}{223} % plot_compstar_rotation.py, ditto, with Prot detected in field star sample
\newcommand{\ncompstarnumerator}{107} % plot_compstar_rotation.py, ditto, with Prot meeting 'set B' criteria in field star sample
\newcommand{\ncompfrac}{16.3\%} % plot_compstar_rotation.py, based on plot_autorotation for defaultcleaning_cutProtColor


\newcommand{\nautovscompstardenominator}{327} % plot_autorotation.py, w/ runid=compstar_NGC_2516. For nequal<=0, nclose<=1, LSP>0.08. 
\newcommand{\nautovscompstarnumerator}{164} % plot_autorotation.py
\newcommand{\nautofrac}{50.1\%} % plot_autorotation.py
...
reported in the Hyades \citep{meingast_hyades_2019,roser_hyades_2019,jerabkova_800_2021},
...
close enough ($d\approx400$\,pc) to facilitate \replaced{photometric
rotation measurements}{measurements of stellar rotation periods} using TESS.  \added{\citet{healy_stellar_2020}
in fact already used TESS rotation periods to study the stellar inclination
distribution of stars in the core of NGC\,2516.}
\replaced{Its halo}{The cluster's halo however} was reported by
\citet{kounkel_untangling_2019} to span
$\approx20\times10\times350$\,pc\added{, which is a larger volume than
has been considered in previous rotation analyses
\citep[][]{Irwin_NGC2516_2007,fritzewski_rotation_2020,healy_stellar_2020}}.
...
\added{To assess whether the stars in the cluster's halo are the same
ages as stars in the cluster's core, we apply three different age-dating
techniques: isochrones, gyrochrones, and lithium depletion
\citep[see][for reviews]{soderblom_ages_2010,soderblom_ages_2014}.
Gyrochronology has now been empirically calibrated for FGK stars with
ages spanning $\approx$10\,Myr to $\approx$4\,Gyr \citep[{\it
e.g.},][]{rebull_usco_2018,curtis_temporary_2019,curtis_rup147_2020,barnes_rotation_2016}.
Lithium equivalent widths can be used for relative age-dating of dwarf
stars over similar age ranges, though for a narrower range of spectral
types \citep{sestito_2005}.  One complication when deriving relative
lithium ages of the K dwarfs comes from the correlation between
lithium abundance and stellar rotation rate, which has recently been
reviewed by \citet{bouvier_lithium-rotation_2020}, and is a subject
...
\begin{figure*}[tp!]
	\begin{center}
		\leavevmode
		\subfloat{
			\includegraphics[width=0.700\textwidth]{f4a.pdf}
		}

    \vspace{-0.7cm}
		\subfloat{
			\includegraphics[width=0.700\textwidth]{f4b.pdf}
		}
	\end{center}
	\vspace{-0.7cm}
  ...
  automated cleaning \replaced{(see Section~\ref{subsubsec:cluster})}{; Set
  $\mathcal{B}$ is the same as Set $\mathcal{A}$, after selecting stars
  below the dashed line in the top panel (see Section~\ref{subsubsec:cluster})}.  The Pleiades
...
\begin{figure}[t]
	...
	\end{center}
	\vspace{-0.7cm}
	\caption{ {\bf Binarity indicators for rotators in \cn.} \replaced{Sets
			$\mathcal{A}$ ({\it top}) and $\mathcal{B}$ ({\it bottom}) are}{Set
...
the \added{Lomb-Scargle} periodogram implementation\deleted{s} in
\texttt{astrobase}\replaced{, namely the traditional Lomb-Scargle, and
the Stellingwerf phase-dispersion minimization algorithm
\citep{lomb_1976,stellingwerf_period_1978,scargle_studies_1982,stellingwerf_period_2011,bhatti_astrobase_2018}}{ \citep{lomb_1976,scargle_studies_1982,bhatti_astrobase_2018}}.
...
pixels.  We recorded the top five periodogram peaks\deleted{ from each
method}, and their corresponding powers.  Finally, as a check on
...
$\mathcal{A}$ and $\mathcal{B}$.  For \replaced{both sets}{Set
$\mathcal{A}$}, we considered light
...
\replaced{light curves}{stars}, out of \nautorotdenominator\ stars \replaced{for which rotation
periods might plausibly have been detected}{that met the initial crowding requirements}.  For Set $\mathcal{B}$, we
additionally required that the Lomb-Scargle \replaced{and Stellingwerf
phase-dispersion methods yielded identical periods to within 10\%}{period
fell below the boundary drawn in Figure~\ref{fig:rot}, which was
constructed using the Pleiades polynomial from
\citet{curtis_rup147_2020}}.
This yielded \nautorotnumeratormatching\ light curves. 
Boolean flags for each set are included in Table~\ref{tab:maintable}.  A
set of field stars was also analyzed for comparison.  The results are
discussed in Appendix~\ref{app:compstar}.
\added{The rationale behind the additional cut in Set $\mathcal{B}$ is
that \cn\ is already known to have few if any stars above the slow
sequence \citep{fritzewski_rotation_2020}.  Stars above the slow sequence
are therefore most likely either not cluster members, or they are cluster
members, but with unresolved binary companions contaminating the rotation
period measurement \citep[{\it
e.g.},][Section~5.1]{stauffer_rotation_2016}.  For the purpose of
assessing which stars are rotationally consistent with being in the cluster,
this is a viable filter; for the purpose of exploring whether any
rotational outliers might be bonafide cluster members, Set $\mathcal{A}$
would be the better cut.}
...
completeness comes at the expense of greater contamination.  \deleted{For
instance, stars above the slow sequence (perhaps field contaminants)
mostly disappear in Set $\mathcal{B}$.  The ``void'' or ``gap''
underneath the slow sequence is also more pronounced in Set
$\mathcal{B}$.}  To facilitate a comparison against the Pleiades and
...
\explain{Figure~\ref{fig:binarity} has been modified to account for the
revised definition of Set B.}
...
($0.5<\bpmrpo<1.3$), there are \replaced{281}{523} stars, of which
\replaced{103 (37\%)}{174 (33\%)} show signs of binarity.  Dividing these
\replaced{281}{523} stars into ``fast'' and ``slow'' sequences by eye,
the fraction of stars showing signs of binarity in the slow and fast
subsamples are \replaced{30\% (63/210)}{27\% (106/289)} and \replaced{56\%
(40/71)}{51\% (68/134)}, respectively.  We speculate on possible
...
\begin{figure*}[tp]
	\begin{center}
		\subfloat{
			...
			\includegraphics[width=0.5\textwidth]{f6b.pdf}
		}

		\subfloat{
			\includegraphics[width=0.9\textwidth]{f6c.pdf}
		}
  ...
  rotation periods in \replaced{Set~$\mathcal{A}$}{Set~$\mathcal{B}$} are shown in blue;  halo
  ...
  ratio of detected to expected stars with rotation periods \added{in Set $\mathcal{B}$} is shown
  ...
  velocity space\added{ (from Set~$\mathcal{A})$}.
...
\explain{This paragraph contained a typo: the sentence containg ``at
least 25\,pc from the cluster core'' was meant to refer to the entire
cluster. }

The results are shown in the top two rows of
Figure~\ref{fig:physical_x_rotn}.  \added{Alternative visualizations
in the canoncial space of observed positions, parallaxes, and proper
motions are presented in Appendix~\ref{app:gaia6d_x_rotn}.}  In
\added{all} these figures, to ensure a fair comparison, we only show
...
of stars were in each bin.  Rotation periods \added{consistent with a
gyrochronological age of 150\,Myr} are detected for
\replaced{66\%}{68\% (471/692)} of stars \replaced{at least 25\,pc
from the cluster core}{reported to be in the cluster}.  The average
detection rate inside of 25\,pc, \replaced{73\%}{72\% (290/404)}, is
slightly higher\added{ than outside of 25\,pc, where the detection
rate is 63\% (181/288)}. Within $\approx$3.5\,pc, the period detection
rate of \replaced{61\%}{59\% (26/44)} is
\replaced{relatively}{anomalously} low, due to crowding and saturation
in the TESS images.
...
rotation periods\added{ in Set $\mathcal{A}$}, $\approx$8 appear as
...
our \added{initial} target list.

\replaced{Even with}{Despite} this level of contamination, the rotation period
...
6708\,\AA\ doublet.  \deleted{This type of age measurement has been reviewed by
...
\begin{figure*}[t]
	...
	\caption{ {\bf Lithium and rotation in NGC\,2516.} Points show all
		candidate \cn\ members with TESS rotation periods\added{ (Set $\mathcal{A}$)} and Gaia-ESO or
...
% UPDATED IN PROOFING
The most likely explanation seems to be that the observed halo structures are
tidal tails \citep[{\it
e.g.},][]{chumak_tails_2006,krumholz_star_2019,dinnbier_tidaltwo_2020}.  As
discussed by \citet{dinnbier_tidalone_2020}, the tail could either be
mostly made of stars released during the gas expulsion phase, or it
could be made of
stars released during the long-term evaporation of the cluster.  In their
N-body simulations, \citet{dinnbier_tidaltwo_2020} found that for the tails to
be as rich as those of \cn, they would need to be dominated by stars
released during an impulsive gas expulsion phase.  The observed
morphology and kinematics of the \cn\ halo also appear to qualitatively agree
with these N-body predictions.
...
\begin{figure*}[t]
  ...
  results of \replaced{\citet{Irwin_NGC2516_2007} (I07) and
  \citet{fritzewski_rotation_2020} (F20)}{\citet{Irwin_NGC2516_2007}
  (I07), \citet{fritzewski_rotation_2020} (F20), and
  \citet{healy_stellar_2020} (HM20)}.
  \replaced{I07 and F20}{I07, F20, and HM20} analyzed stars in the
  core of \cn.
  \label{fig:litcomp}
	}
\end{figure*}
...
\explain{Figure~\ref{fig:litcomp} has been updated to include rotation
periods from \citet{healy_stellar_2020}.}
...
from \replaced{``Class 1 and 2'' \citet{fritzewski_rotation_2020} rotators, and
from the rotators in \citet{Irwin_NGC2516_2007}}{``Class 1 and
2'' \citet{fritzewski_rotation_2020} sources, from
Table 1 of \citet{healy_stellar_2020}, and
from \citet{Irwin_NGC2516_2007}}.  The main
...
slow and fast rotation subsamples are \replaced{30\% (63/210) and 56\%
(40/71)}{26\% (106/289) and 51\% (68/134)}; the fast rotators show a
preference for binarity.  This correlation is in line with the earlier
findings noted above.  While many rapid rotators in \cn\
(\replaced{34\%}{66\%}) have no evidence for being binaries, the cuts we
...
\citet{bouvier_lithium_2008} and
\citet{eggenberger_impact_2012} have shown that longer disk lifetimes
...
periods seen in \cn\ \deleted{across both Sets $\mathcal{A}$ and
$\mathcal{B}$}
...
\begin{itemize}
  ...
  \item {\it Bonafide \cn\ members exist out to $\approx$250\,pc in
    ...
    assessment.
   	\added{The majority of the halo members do however exist within
  ...
  \item {\it The field star contamination rate increases at larger
    ...
    $\approx$50\%.  \replaced{Given the number density ratios of cluster to field stars at the widest separations ($\approx 10^{-3}$, \citealt{meingast_2021}), these contamination fractions are not as bad as one might fear.}{While the latter contamination fraction may seem high, a comparison sample of field stars (Appendix~\ref{app:compstar}) has a detection rate three times lower, and a  distribution in the period-color plane inconsistent with that of a population with a single age.}
   %
...
\begin{figure}[b]
	\begin{center}
		\leavevmode
		\includegraphics[width=0.3\textwidth]{f11.pdf}
	\end{center}
	\vspace{-0.7cm}
...
\begin{figure}[t]
  ...
  stars are both subject to the same \deleted{``Set $\mathcal{A}$''}
  period-measurement and cleaning procedures.
  \added{The dashed line is as in Figure~\ref{fig:rot}}.
  \label{fig:compstar}
	}
\end{figure}
...
\explain{Numbers in this paragraph have been updated per the revised
definition of Set $\mathcal{B}$. }
...
for crowding resulted in \ncompstardenominator\ \added{field} stars
for which rotation periods could have been detected.  Imposing the
same Lomb-Scargle power cutoff (${\rm LSP}>0.08$) and \replaced{period
upper limit ($P<15\,{\rm days}$)}{color-period cutoff as that used to
define Set $\mathcal{B}$} yielded \ncompstarnumerator\ period
detections (\ncompfrac).  Within the same \replaced{brightness
cutoff}{color range}, \nautovscompstarnumerator\ of
\nautovscompstardenominator\ \replaced{cluster stars}{kinematically
identified candidate cluster members} yielded period detections
(\nautofrac).  \replaced{While the detection fractions are only mildly different
(likely due to the brightness cutoff), the }{The }period--color distributions
are \replaced{very}{also quite} different\added{: the candidate
cluster members show an overdensity coincident with the expected
gyrochronal age, while the field stars show a much broader rotation
period distribution} (Figure~\ref{fig:compstar}).  This confirms our
...
\begin{figure}[t]
	\begin{center}
		\leavevmode
		\subfloat{
			\includegraphics[width=0.49\textwidth]{f14a.pdf}
			\includegraphics[width=0.49\textwidth]{f14b.pdf}
		}
	\end{center}
	\vspace{-0.5cm}
...
\added{Figure~\ref{fig:gaia6d_x_rotn} is an alternative visualization
of the data shown in Figure~\ref{fig:physical_x_rotn}.  The rotation
periods in this diagram correspond to ``Set $\mathcal{B}$'' as
described in Section~\ref{subsec:tess}.  One issue with the
visualization as displayed however is that the layering hides the 
non-rotators toward the cluster center: we did not detect rotation
periods for roughly one in four stars at the cluster center (see
upper-right panel in Figure~\ref{fig:physical_x_rotn}).  }
% 
% 
% 
